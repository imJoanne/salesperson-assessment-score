...
export default function DashboardPage() {
  ...
  return (
    <main className="min-h-screen px-4 py-6 sm:px-8 lg:px-12">
      ...
      <section className="mx-auto max-w-7xl py-12"><p className="text-xs font-bold tracking-[.18em] text-[#174b36]">OPERATOR VIEW · LIVE DATA</p><div className="mt-3 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="text-4xl font-semibold tracking-[-.05em] sm:text-6xl">Assessment dashboard</h1><p className="mt-3 text-[#66736d]">A live view of every anonymous report, purchase, and rating.</p></div><span className="w-fit rounded-full bg-[#d9ede1] px-4 py-2 text-xs font-bold text-[#174b36]">● SUPABASE CONNECTED</span></div>
        ...
        <section id="recent-reports" className="mt-5 overflow-hidden rounded-[1.6rem] border border-[#dfe3dc] bg-[#fffefa]"><div className="flex items-center justify-between border-b border-[#e5e7e2] p-6 sm:px-8"><div><p className="text-xs font-bold tracking-[.15em] text-[#66736d]">RECENT ACTIVITY</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.03em]">Latest reports</h2></div><Link href="/" className="text-sm font-bold text-[#174b36]">Run another →</Link></div><div className="divide-y divide-[#e5e7e2]">{metrics.recentReports.map((report) => <Link href={`/reports/${report.id}`} key={report.id} className="grid gap-2 p-5 transition hover:bg-[#f7f8f4] sm:grid-cols-[1fr_auto_auto] sm:items-center sm:px-8"><div><span className="font-bold capitalize">{report.lead_type} lead</span><span className="ml-3 text-sm text-[#66736d]">{report.gap_flags.length} gaps</span></div><span className="text-sm text-[#66736d]">Score {report.total_score}</span><span className="text-sm text-[#66736d]">{new Date(report.created_at).toLocaleDateString()}</span></Link>)}</div></section>
      </section>
    </main>
  );
}
