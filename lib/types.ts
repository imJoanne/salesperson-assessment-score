...
export type LeadProfile = { full_name: string; email: string; company: string | null; phone: string | null };
export type UserProfile = LeadProfile & { id: string; role: "visitor" | "operator" };
export type AssessmentResponse = { id: string; answers: SelectedAnswer[]; total_score: number; lead_type: LeadType; gap_flags: string[]; created_at: string; lead?: LeadProfile | null };
...
export type DashboardMetrics = {
  ...
  recentReports: Pick<AssessmentResponse, "id" | "lead_type" | "total_score" | "gap_flags" | "created_at" | "lead">[];
};
